...
public class ChiTietSanPhamBUS {
    ...
    private final ChiTietSanPhamDAO ctspDAO = ChiTietSanPhamDAO.getInstance(); // Use singleton instance
    private final PhienBanSanPhamBUS pbspbus = new PhienBanSanPhamBUS();
    private ArrayList<ChiTietSanPhamDTO> listctsp = new ArrayList<>();

    public ChiTietSanPhamBUS() {
    }

    public ArrayList<ChiTietSanPhamDTO> getAllByMaPBSP(int pbsp) {
        listctsp = ctspDAO.selectByPb(pbsp); // Correct method name
        return listctsp;
    }
    ...
    public ChiTietSanPhamDTO getByIndex(int index) {
        if (index >= 0 && index < this.listctsp.size()) {
    ...
    public HashMap<Integer, ArrayList<ChiTietSanPhamDTO>> getChiTietSanPhamFromMaPN(int maphieunhap) {
        ...
        for (ChiTietSanPhamDTO i : chitietsp) {
            result.computeIfAbsent(i.getMaphienbansp(), k -> new ArrayList<>()).add(i);
        }
        return result;
    }
    ...
    public void updateXuat(ArrayList<ChiTietSanPhamDTO> ct) {
        for (ChiTietSanPhamDTO chiTietSanPhamDTO : ct) {
            ctspDAO.updateXuat(chiTietSanPhamDTO); // Ensure this method exists in DAO
        }
    }
    ...
    // Phương thức đã sửa
    public ArrayList<ChiTietSanPhamDTO> FilterPBvaTT(String text, int masp, int maphienbansp, int tinhtrang) {
        return filterPBvaTT(text, masp, maphienbansp, tinhtrang);
    }

    public ArrayList<ChiTietSanPhamDTO> FilterPBvaAll(String text, int masp, int maphienbansp) {
        return filterPBvaAll(text, masp, maphienbansp);
    }
}
